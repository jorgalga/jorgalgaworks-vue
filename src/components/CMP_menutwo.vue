  ...
  <div class="component-wrapper cmpmenutwo" v-bind:class="$props.type">
    ...
    <div ref="menucontainer" class="component-container hidden">
      <nav>
        <ul>
          <li>
            <router-link v-bind:to="'/' + clang" v-html="data.copy[clang].menu_links.home"></router-link>
          </li>
          <li>
            <router-link v-bind:to="'/reglas/' + clang" v-html="data.copy[clang].menu_links.rules"></router-link>
          </li>
          <li>
            <router-link
              v-bind:to="'/proposito/' +clang"
              v-html="data.copy[clang].menu_links.purpose"
            ></router-link>
          </li>
          <li>
            <router-link
              v-bind:to="'/teams/' + clang"
              v-html="data.copy[clang].menu_links.teams"
            ></router-link>
          </li>
          <li>
            <router-link
              v-bind:to="'/ranking/' + clang"
              v-html="data.copy[clang].menu_links.ranking"
            ></router-link>
          </li>
          ...
          <li>
            <a
              target="_blank"
              v-bind:href="data.webex_url"
              v-html="data.copy[clang].menu_links.webex"
            ></a>
          </li>
          <li>
            <a
              target="_blank"
              href="mailto:platformrevolution@minsait.com"
              v-html="data.copy[clang].menu_links.contact"
            ></a>
          </li>
          <li>
            <a
              target="_blank"
              href="http://blog.onesaitplatform.com/"
              v-html="data.copy[clang].menu_links.blog"
            ></a>
          </li>
        </ul>
      </nav>
      <div class="lang-selector">
        <div class="text-button">
          <a @click.prevent="toggleLang('es')" ref="link-es" href class="lang-link">ES</a> |
          <a @click.prevent="toggleLang('en')" ref="link-en" href class="lang-link">EN</a>
        </div>
      </div>
      <div
        class="btn white participa"
        @click.prevent="goTo('/?participa')"
        v-html="data.copy[clang].menuparticipabtn"
      ></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    resizeHandler () {
      ...
      this.scaleR = Math.max(
        this.data.mobile_width / this.data.max_width,
        w / this.data.max_width
      )
      this.scaleR = Math.min(
        this.scaleR,
        this.data.max_width_limit / this.data.max_width
      )
    },
    goTo2 (site) {
    ...
    goTo (site) {
      if (document.getElementsByClassName('home').length > 0) {
        ...
        document
          ...
          .scrollTo(0, document.getElementsByClassName('cmpform')[0].offsetTop)
      } else {
        console.log('to Site')
        window.location.href = '/#' + site
      }
